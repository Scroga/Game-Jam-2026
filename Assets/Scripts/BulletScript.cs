    ...
    public class BulletScript : MonoBehaviour {
        ...
        private void OnEnable() {
            ...
            if (gunScript.gunObject.bulletSettings.explosiveBullets == false) {
                lifeCoroutine = StartCoroutine(RemoveBulletAfterTime(gunScript.gunObject.bulletSettings.bulletLife));
            }
            else {
                lifeCoroutine = StartCoroutine(ExplodeBulletDelay(gunScript.gunObject.bulletSettings.bulletLife));
            }
        }
        ...
        private void ObjectCollision(GameObject collision, Vector2 contactPosition) {
            if (gunScript.gunObject.bulletSettings.explosiveBullets == true) {
                ...
                if (collision.TryGetComponent(out HealthScript healthScript)) {
        ...
        private void ExplodeBullet() {
            ...
            foreach (Collider2D col in hitColliders) {
                if (col.gameObject.TryGetComponent(out HealthScript healthScript)) {
                    healthScript.RemoveHealth(damage); //Remove health if needed
                }
            }
